...
LoadLibraries <- function() {
  myLibraries <- c("knitr", "ggplot2", "faraway", "sqldf", "corrplot", "tidyverse",
                   "gridExtra", "PerformanceAnalytics")
...
```{r}
modelo <- lm(formula = data_cuantitativa$Colesterol ~ data_cuantitativa$Edad, data=data_cuantitativa)
summary(modelo)
...
```{r}
plot(data_cuantitativa$Colesterol ~ data_cuantitativa$Edad, col="blue", xlab= "Edad", ylab="Colesterol", main="Recta regresión Edad y Colesterol")
abline(modelo)
```
...
Para ver si podemos resolver parte del problema, realizaremos una regresión multiple. Empezaremos calculando el coeficiente de correlación de Pearson para las distintas convinaciones. Lo hemos calculado con anterioridad, pero vamos a hacerlo de nuevo para este nuevo paso.
```{r}
correlación <- round(cor(data_cuantitativa, method = "pearson"), digits = 2)
kable(correlación)
```
...
```{r}
mrm <- lm(data_cuantitativa$Colesterol
          ~data_cuantitativa$Edad+data_cuantitativa$P.sis+data_cuantitativa$P.diast+data_cuantitativa$IMC+data_cuantitativa$`N/d cigarrillos`)
summary(mrm)
```
...
```{r}
mrm_predictores <- (lm(formula = data_cuantitativa$Colesterol ~ data_cuantitativa$Edad + 
    data_cuantitativa$P.diast + data_cuantitativa$IMC + data_cuantitativa$`N/d cigarrillos`))
summary(mrm_predictores)
```
En este caso, hemos quitado la variable de presión sistolica. Aun asi, los datos obtenidos en la regresión multiple no han mejorado. 
