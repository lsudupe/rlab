---
title: "PEC 3"
author: "Andrea Sanchez Rodriguez, Laura Sudupe Medinilla"
date: "Dic 14, 2020"
output:
  pdf_document:
    toc: yes
...
Los datos seleccionados para esta PEC proceden del package `faraway` de `R`. Son del Western Collaborative Group Study. Se trata de un dataset con nombre `wcgs` de 3145 hombres de entre 39 y 59 años de San Francisco. Todos ellos estaban libres de padecer una enfermadad coronaria y lo que se ha hecho es evaluar pasados 10 años si esta situación había cambiado a la vez que se midieron algunas características físicas y clínicas de estos hombres.

Más concretamente, el WCGS comenzó en 1960 con 3.524 voluntarios varones que eran empleados de 11 empresas de California. Los sujetos tenían entre 39 y 59 años y no tenían enfermedad cardíaca según lo determinado por un electrocardiograma. Después de la selección inicial, la población del estudio se redujo a 3154 y el número de empresas a 10 debido a diversas exclusiones. La cohorte estaba compuesta por empleados de dos grupos. Como línea base se recopiló la siguiente información: sociodemográfica, incluida la edad, la educación, el estado civil, los ingresos, la ocupación; físico y fisiológico, incluyendo altura, peso, presión arterial, electrocardiograma y arco corneal; bioquímicos que incluyen fracciones de colesterol y lipoproteínas; antecedentes médicos y familiares y uso de medicamentos; datos de comportamiento que incluyen entrevistas tipo A, tabaquismo, ejercicio y consumo de alcohol. Las encuestas posteriores agregaron datos sobre antropometría, triglicéridos, encuesta de actividad de Jenkins y uso de cafeína. El seguimiento promedio continuó durante 8.5 años con exámenes repetidos.
...
Podemos comprobar si existen valores faltantes en el conjunto de datos:
...
for(i in 1:length(data$chol))
  if (data$fumador[i]=="si"&& (data$chol[i] > mean(data$chol))) {
    chol_si_fumador <- chol_si_fumador + 1
  } else if (data$fumador[i]=="no" && (data$chol[i] > mean(data$chol))) {
    chol_no_fumador <- chol_no_fumador + 1
  }

porcentaje_chol_nofumador <- (chol_no_fumador/length(data$chol[data$chol>mean(data$chol)]))*100
porcentaje_chol_sifumador <- (chol_si_fumador/length(data$chol[data$chol>mean(data$chol)]))*100
porcentaje_chol_nofumador
porcentaje_chol_sifumador
```

Vemos que para los sujetos del estudio con colesterol superior a la media, el 48% no son fumadores y el 52% sí lo son. Es decir, los fumadores presentan valores de colesterol ligeramente superiores.
...
porcentaje_chol_nofumador2 <- (chol_no_fumador_edad/length(data$chol[data$age>mean(data$age)]))*100
porcentaje_chol_sifumador2 <- (chol_si_fumador_edad/length(data$chol[data$age>mean(data$age)]))*100
porcentaje_chol_nofumador2
porcentaje_chol_sifumador2
```

Vemos que no hay mucha diferencia entre ser fumador y no serlo, teniendo en cuenta los hombres que están por encima de la media de edad. Puede que en este estudio no sea relevante pero si hubiesemos obtenido porcentajes más distantes indicaría que sí que hay una relación a tener en cuenta.
...
En la regresión lineal se asume que los residuos del modelo tienen una distribución normal. Se puede verificar con la gráfica Q-Q normal.
Se cumplan o no estas condiciones, vemos que no tiene porque existir una correlación alta entre las variables estudiadas. 
...
```{r}
boxplot(df_anova$imc ~ df_anova$tipo_chd,
        main = "Boxplot de IMC en relación al grupo de CHD",
        ylab = "Imc",
        xlab = "Tipo CHD",
...
Si la ANOVA hubiese resultado con un p-valor inferior a 0.05, podríamos estudiar si hay diferencias significativas entre dos grupos. Esto, podemos verlo mediante métodos de comparaciones múltiples y correcciones y se haría de la siguiente manera:
...
Si el p-valor (p adj) es menor que el 5% tenemos diferencias significativas, no es nuestro caso como ya veíamos a partir de la ANOVA.
...
df_jerarquico <- data[, c(1,4,5,6,8,11,15)] #Aislar los datos
#A continuación, nombramos las columnas
colnames(df_jerarquico) <- c('Edad', 'P.sis', 'P.diast', 
                                 'Colesterol', 'N/d cigarrillos', 'Tipo CHD', 'IMC')
...
pairs(df_jerarquico, col=df_jerarquico$'Tipo CHD')
```
...
Al hacer un análisis descriptivo de los datos, los principales estadísticos nos indican que el rango de edad de los pacientes del estudio no es muy disperso. Además, con el diagrama de tallo y hojas vemos que se concentra entre los 39-44 años y la edad se distribuye de forma decreciente (hay más jóvenes que mayores). Seguramente se debe al tipo de trabajo que realizan los hombres de este estudio. Esto ya nos da una vista preliminar de que el estudio es para un grupo concreto de la población. Al haber creado una variable `IMC` a partir del peso y la altura, hay una estrecha correlación entre ellas. Por lo tanto, no nos hemos centrado en estudiar las relaciones entre estas variables a lo largo del estudio.

Con la herramienta SQL hemos observado que los hombres con mancha en la pupila presentan niveles de colesterol altos, teniendo en cuenta que la media es de 226.35, es una observación a tener en cuenta en la evaluación visual del paciente. Además mediante la visualización gráfica concluimos que existen diferencias entre las medias de cholesterol de los hombres con arco senil y sin él, siendo mayor el primero. Con la tabla de contingencia cruzada verificamos que en el grupo que tanto si se ha desarrollado el CHD como si no, es más frecuente no encontrar arco senil, aunque en el grupo que sí tiene CHD la diferencia es menos clara. En conclusión, podemos relacionar la aparición del arco senil con la enfermedad, aunque puede haber otros efectos confusores como la edad. Las variables que más se diferencian en la aparición/no aparición de CHD es la edad, las presiones sanguíneas y el número de cigarrillos fumados. 

Mediante el análisis de regresión lineal simple podemos decir que hay una asociación estadística significativa entre nuestras dos variables (colesterol en función de la edad) aunque el valor de $R^2$ sea muy pequeño. Un rango mayor en la edad de los pacientes nos ayudaría más a vislumbrar la relación real entre la Edad y el colesterol. Con la regresión múltiple y seleccionando las variables con el método step seguimos obteniendo muchas variables significativas, pero con el coeficiente de determinación muy bajo. Aunque hayamos seleccionado variables que es sabido que aumentan la concentración de colesterol, no hemos podido verificarlo con los datos del estudio. Aun así, esto nos puede servir para dar información sobre un mejor diseño de los pacientes del estudio. Variar la población en el rango de edad, hacer un estudio análogo con mujeres... aunque en este caso el estudio se centraba en este grupo de hombres de estas características.

Con el estudio de probabilidades y centrándonos en la variable de colesterol vemos que para la distribución teórica si fuese una normal el valor de colesterol que deja por debajo el 95% de los hombres del estudio sería de 298 aproximadamente y para nuestros datos es 302, son valores muy parecidos como ya intuíamos por la similitud entre la distribución empírica y la Normal. En 29 términos de este estudio, podríamos catalogar a los individuos por encima de este valor de referencia como grupo de riesgo por tener el colesterol más alto al 95% de los hombres del estudio. Mediante la aproximación de la variable CHD a una binomial podemos hacer suposiciones del alcance de la enfermedad.

Por último, para comparar estadísticamente si los tipos de CHD son diferentes en cuanto al IMC de los pacientes hemos realizado un test ANOVA. Mediante la grafica boxplot vemos que los datos tienen una distribución parecida en cada grupo, observación respaldada por el ANOVA que nos dice que el p valor no es significativo. Mediante la realización de clusters observamos que no hay equilibrio en los grupos, pudiendo deberse a alguna variable que tenga muchos valores dispersos.
