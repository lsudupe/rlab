...
```{r}
boxplot(health_heart$chol, col="blue")
```
...
alcanzada).
```{r}
plot(health_heart$thalach, col="blue", xlab="Resting electro", ylab ="Y")
```
...
```{r}
library(ggplot2)
qplot(x=age, y=chol, data=health_heart, geom="point")
```
...
obtenerse?
```{r}
modelo <- lm(health_data$thalach~health_data$age, data=health_data)
summary(modelo)
plot(health_data$age, health_data$thalach)
abline(modelo)

```
```{r}
cor.test(health_data$age, health_data$thalach,
                               method="pearson")
```
...
```{r}
age <- c(health_data$age)
thalach <- c(health_data$thalach)
...
e) De los apartados anteriores, ¿qué se puede concluir? No existe correlación directa entre las variables seleccionadas.
