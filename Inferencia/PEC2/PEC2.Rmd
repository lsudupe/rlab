...
```{r}
# necesitamos un data.frame
dfbootstrap <- data.frame(dades$tas) 

media <- function(dfbootstrap, indices)
{
  d <- dfbootstrap[indices,]
  mean(d)
}

replicas <- boot(data= dfbootstrap, statistic =media, R=5000)
```

Vemos los elementos de replicas
```{r}
names(replicas)
```
Calculamos el sesgo
